...
func createMariadbPasswordSecret(client coreclientv1.SecretsGetter, config *OperatorConfig) error {
	...
	if apierrors.IsNotFound(err) {
		...
		mariadbPassword := generateRandomPassword()
		secret := &corev1.Secret{
			...
			StringData: map[string]string{
				baremetalSecretKey: mariadbPassword,
			},
		}

		glog.V(1).Infof("Maridb password does not exist so creating one.")
		_, err := client.Secrets(config.TargetNamespace).Create(secret)
		return err
	}
	if err != nil {
		glog.Errorf("Error while looking for existing Mariadb password : %v", err)
		return err
	}
	// Secret already exists so do nothing.
	glog.V(3).Info("Maridb password already exists.")
	return nil
}
